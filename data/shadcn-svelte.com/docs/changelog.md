...
- [Spinner](components/spinner) : An indicator to show a loading state.
- [Kbd](components/kbd) : Display a keyboard key or group of keys.
- [Button Group](components/button-group) : A group of buttons for actions and split buttons.
- [Input Group](components/input-group) : Input with icons, buttons, labels and more.
- [Field](components/field) : One component. All your forms.
- [Item](components/item) : Display lists of items, cards, and more.
- [Empty](components/empty) : Use this one for empty states.
...
Check out the [Calendar](components/calendar) docs page and the [Calendar Blocks](../blocks/calendar) page for more.
...
This release includes several key changes, all outlined in the [Tailwind v4 migration guide](migration/tailwind-v4). If you're still using Svelte v5 with Tailwind v3, your project and the CLI will continue to work as expected until you're ready to upgrade.
...
We've added support for custom/remote registries - This means you can publish your own components and share them with the community via the `shadcn-svelte` CLI. See the [Registry documentation](registry) for more information.
...
We've added a new component to the project, [Breadcrumb](components/breadcrumb).

### New Component: Scroll Area

We've added a new component to the project, [Scroll Area](components/scroll-area), which is built on top of the [Scroll Area](https://bits-ui.com/docs/components/scroll-area) component from Bits UI.
...
We've added a new component to the project, [Resizable](components/resizable), which is built on top of [PaneForge](https://paneforge.com). PaneForge is still in an early stage, so be sure to raise any issues you find with the library on the [PaneForge GitHub](https://github.com/svecosystem/paneforge).
...
We've added four new components to the project, [Carousel](components/carousel), [Drawer](components/drawer), [Sonner](components/sonner), & [Pagination](components/pagination).

### New Component: Carousel

We've added a new component to the project, [Carousel](components/carousel).
...
We've added three new components to the project, [Calendar](components/calendar), [Range Calendar](components/range-calendar), & [Date Picker](components/date-picker).

## November 2023

### New Component: Toggle Group

We've added a new component to the library, [Toggle Group](components/toggle-group).

## October 2023

We've added two new components to the library, [Command](components/command) & [Combobox](components/combobox). We've also made some updates to the `<Form.Label />` component that you'll want to be aware of.
...
Be sure to check out the [Command](components/command) docs for more information.
...
Be sure to check out the [Combobox](components/combobox) docs for more information.
